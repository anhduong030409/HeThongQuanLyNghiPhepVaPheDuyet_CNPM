...
// ===== CREATE =====
    ...
    // Tao loai phep
    $sql  = "INSERT INTO leave_types (name, max_days_per_year, is_paid, requires_document, carry_over_days, is_active)
             VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "siiiii", $name, $max_days, $is_paid, $requires_document, $carry_over_days, $is_active);
...
// ===== UPDATE =====
    ...
    $sql  = "UPDATE leave_types SET name=?, max_days_per_year=?, is_paid=?, requires_document=?, carry_over_days=?, is_active=?
         WHERE id=?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "siiiiii", $name, $max_days, $is_paid, $requires_document, $carry_over_days, $is_active, $id);
