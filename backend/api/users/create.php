...
if (!$role_id) {
    ...
    echo json_encode(["status" => "error", "message" => "Thieu role_id"]);
    exit;
}
if (!$full_name || !$email) {
    http_response_code(400);
...
$sql = "INSERT INTO users (full_name, email, password_hash, phone, role_id, department_id, manager_id)
        VALUES (?, ?, ?, ?, ?, ?, ?)";
